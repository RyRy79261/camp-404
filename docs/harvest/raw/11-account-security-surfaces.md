...
**One direction-of-flow note that matters here.** The donor's sanitization module explicitly credits Camp 404 (`account-sanitization.ts:1`, `:23-24`). Camp 404 already ships `packages/db/src/account.ts` + `apps/web/lib/account.ts:deleteAccount` with a `Lost Cat #N` stub and a written-but-partial deletion spec (`docs/specs/2026-05-30-account-deletion-design.md`). So the deletion half of this unit is a **re-import of a Camp 404 pattern that grew a 14-day grace period, a guard system, a sweeper and 771 lines of tests in the donor** — not a net-new import.
