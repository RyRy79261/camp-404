...
- **Adding a team key (Phase 4), done for Finance and three more.** Commit
  ...
  `packages/db/src/__tests__/new-teams-migration.test.ts`. `teamEnum` now has
  **12** values. The team-interest questions are optional, so a new team does
...
> HR and Mutant Vehicle (#236) made twelve. The owner ruled that teams are
...
> **[CORRECTION 2026-09-24]** `teamEnum` now has **12** values: the eight above
> plus `finance` (migration `0039`) and `transport_and_logistics`,
> `communications_and_hr` and `mutant_vehicle` (migration `0044`). A grep of
