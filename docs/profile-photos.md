...
> Photos are stored in Vercel Blob and surfaced as a large circular avatar
> on the profile page and a small one in the home header.
...
3. **Storage.** `app/api/uploads/avatar/route.ts` authenticates the user,
   ...
   image (`image/*`, ≤ 5 MB), and writes it to Vercel Blob under
   `avatars/<userId>/…` with `access: "public"`. It returns `{ url }`.
4. **Persistence.** The returned URL is stored on `users.profile_image_url`
...
5. **Display.** The `@camp404/ui` `Avatar` component renders the photo with
   an initials fallback. Used on `/profile`, `/profile/edit`, and in the
   home header (which links to `/profile`).
...
- `BLOB_READ_WRITE_TOKEN` — Vercel Blob read/write token (see
  `.env.example`). When it is unset, or when `E2E_TEST_MODE=1`, the upload
  route skips the network call and returns a deterministic stub URL so local
  dev and E2E tests work without a configured Blob store.

## Notes

- The avatar preview uses a plain `<img>` inside a rounded container rather
  than `next/image`; switching to `next/image` later would mean allowlisting
