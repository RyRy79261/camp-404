/**
 * Client-side validation for the ID document number field on the
 * burner-profile questionnaire. The shape depends on the toggle the
 * user picked one field earlier — South African ID vs Passport.
 *
 *   SA ID — 13 digits, with a valid YYMMDD date prefix and a Luhn
 *   check digit (Department of Home Affairs spec). The Luhn variant
 *   here splits odd / even positions and doubles the concatenated
 *   even-position number, not each digit, which is the SA-specific
 *   wrinkle versus standard Luhn.
 *
 *   Passport — strict format check is impractical because Camp 404
 *   is international (Berlin, London, Cape Town crew). Accept any
 *   alphanumeric run of 6–12 characters; the helper text below the
 *   field tells South Africans what their format looks like.
 */
