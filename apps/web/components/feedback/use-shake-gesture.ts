 ...
 * must be initiated by a user gesture. `DeviceMotionEvent.requestPermission`
 * only exists on those browsers — elsewhere motion fires without a prompt.
 */
...
export function useShakeGesture({
  ...
  onShake,
  threshold = 8,
  // Default of five jolts within the window — a deliberate, vigorous shake,
  // not an accidental bump, is needed to open the report dialog.
  requiredJolts = 5,
  windowMs = 800,
  cooldownMs = 3000,
