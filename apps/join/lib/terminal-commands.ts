// TERMINAL's commands on join.camp-404.com: what each one answers, and the
// easter eggs. The shell that runs them (parsing, clear, exit, the 404 for an
// unknown word, history) is @camp404/os/terminal's; these read Join's own
// fee, copy and team data.
...
function cat(target: string, data: JoinData): TermResult<AppId> {
...
const hello = () => ({ lines: [out("Hello, lost one.")] });

export const JOIN_COMMANDS: CommandTable<AppId, JoinData> = {
  // Easter eggs first: they are whole phrases, or any use of a word.
  phrases: [
    ({ lower }) =>
      lower === INKBLOT.password
        ? {
        ...
        : undefined,
    ({ lower }) =>
      lower === "sudo coup chef" || lower === "sudo overthrow chef"
        ? {
            lines: [
        ...
        : undefined,
    ({ lower }) =>
      lower === "walk duck" || lower === "walk the duck"
        ? { lines: DUCK.map(out) }
        : undefined,
    ({ cmd }) =>
      cmd === "sudo"
    ...
    ({ cmd }) =>
      cmd === "rm"
        ? {
            lines: [
              err(
                "MOOP detected. Nothing leaves a trace here, not even files.",
              ),
            ],
          }
        : undefined,
    ({ lower }) =>
      lower === "404" ? { lines: [hi("ERROR 404: YOU ARE HERE")] } : undefined,
    ({ cmd }) =>
      cmd === "meow"
        ? { lines: [out("Now Now Meow Meow is warming up its engine. =^.^=")] }
        : undefined,
    ({ cmd, lower }) =>
      cmd === "coffee" || lower === "make coffee"
        ? { lines: [err("418: I'm a teapot. Bring your own coffee gear.")] }
        : undefined,
  ],
  commands: {
    help: () => ({
    ...
    whoami: () => ({
      lines: [
        out("uid=404(burner) gid=404(misfits) groups=lost,tomfoolery,nonsense"),
        out("You are lost. That's the point. You're in the right place."),
      ],
    }),
    pwd: () => ({ lines: [out("/tankwa-town/plot-43/blanket-fort")] }),
    cd: () => ({
...
export function runCommand(input: string, data: JoinData): TermResult<AppId> {
  return run(input, JOIN_COMMANDS, data);
}
