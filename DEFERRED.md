...
- **Gate fallback removal** — `memberBlock` in `apps/web/lib/member-gate.ts` keeps a belt-and-braces `completedAt` check beside the `required_actions` gate. The MCP completion hooks have landed. Remove the check once an operator has run `camp404 backfill-required-actions` in prod. **[E]**
- **Redirect-ladder consolidation (captain pages)** — member pages (home, `tools/*`, `family-tree`, `profile/*`) share `requireMemberPage()`. The `captains/*` pages still walk their own ladder. **[audit #7, E]**
...
- **Burner profile gate backfill:** run `camp404 backfill-required-actions` once in prod. It gives members who joined before signup seeding their `burner_profile` gate row (finished profiles get a completed row). Safe to re-run. After it has run, the `completedAt` fallback in `apps/web/lib/member-gate.ts` can go.
