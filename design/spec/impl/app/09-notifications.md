...
| 4 | **Pagination / list length.** Resolved 2026-09-16: `listInbox(userId, { before, limit })` returns `{ items, nextCursor }`, 30 at a time; `InboxFeed` loads older pages on scroll through `loadOlderNotificationsAction`. | `09-notifications.md` OQ4 | Owner chose paging with lazy load. |
