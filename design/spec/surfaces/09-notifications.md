...
| Tap a row | A row about something else opens it: an announcement opens `/announcements/[id]`, a questionnaire notice opens `/questionnaires/[activationId]` (`notificationLink` in `@camp404/core`, which the push message uses too). A row about nothing else stays a static row. No dismiss, no per-row mark-read/unread |
...
Note: `refType` and `refId` are on the item so a row can link to what it is about. `channel`, `pushStatus` and `broadcastId` are omitted.
...
- **Paged, newest first (2026-09-16).** `listInbox` returns 30 rows at a time on a `(created_at, id)` cursor, served by `notification_deliveries_user_created_idx`. The server renders the first page; the next page loads as the member scrolls near the end, or on "Load older". Each page's rows are marked read when they load, so an older unread notice keeps its badge until it is shown. Rows sit under camp-day headings ("Today", "Yesterday", then the date, in `CAMP_TIME_ZONE`).
...
4. **Pagination / list length.** Resolved (owner's call, 2026-09-16): page the inbox and load more as the member scrolls. See Edge cases.
5. **`refType` / `refId` deep-link.** Resolved (2026-09-16): rows link through `notificationLink`. An announcement opens `/announcements/[id]`, where the member's delivery row is the permission: no delivery, a 404, without reading the broadcast.
