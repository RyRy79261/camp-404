...
Terminal `RejectScrim`/`RejectConfirm` (440px, `$destructive` stroke) — mobile `RejectModal`. Title bar `triangle-alert` "Reject application". Body: "Reject {Name}'s application?" + "They'll be told the application wasn't approved. This can't be undone here." Actions: `Keep pending` (`Button-Outline`) + `Reject` (`Button-Primary`, `$destructive` fill). **Changed 2026-09-16 (owner's call: decisions can be reversed):** the second sentence is now "You can re-open it later." The same dialog also serves "Remove from camp" for an approved member (they lose access and come off this year's teams) and a bulk "Reject N applications" from the Pending filter, with one optional reason. The profile panel lists every decision from the member's status (Approve / Reject or Remove from camp / Move back to pending); refused ones stay visible and disabled with the reason. Pending rows can be ticked and decided together. None of these extras is drawn.
