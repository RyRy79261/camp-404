...
- **`completedAt` overwrite on replay.** `upsertBurnerProfile` with `markComplete:true` sets `completed_at = now()`, not preserving the original. The inline comment in `lib/forms.ts` claims idempotency, but the SQL disagrees. This means `completedAt` will drift toward the most-recent replay date. Affects "Last edited" display only if `updatedAt` is somehow null (it is always set). **⛔ Must resolve before build (data integrity):** preserve the original `completed_at` (set it only when previously null) unless the team explicitly accepts the drift and updates the `lib/forms.ts` comment to match — do not leave the SQL and the idempotency comment contradicting each other.
