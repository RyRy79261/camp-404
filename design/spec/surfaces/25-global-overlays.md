...
- **"Report a bug" and "Request a feature" on /profile, and "Report" on the route error page** — open the same dialog through `openReportProblem` (owner's call, 2026-09-16: no floating button). **[CORRECTION 2026-09-22]** The single "Report a problem" link on /profile became two typed buttons on the report-settings card, which says what a report sends before it is sent. Those two buttons appear only while `filing` is `"ok"`: with `no_token` or `bad_repo` the card renders none of them (not hidden — absent) and leads with the reason and the sentence a send would come back with, while the disclosure below it still describes what a report would attach once it is fixed. Shaking the phone still opens the reporter either way. The error page prefills the trace code and shows Report only when signed in.
