...
| E3 | ~~**Rate-limit persistence.**~~ Resolved in B4: limits count in Postgres (`action_rate_limit`), shared by every instance. The in-memory bucket is only the fallback when the database cannot store a count. | eng | med | 03 OQ3 |
