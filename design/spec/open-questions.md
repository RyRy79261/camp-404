...
**Totals:** 136 items — 0 blocker · 12 high · 50 med · 74 low (E15 family-tree cycle guard + D16 `completedAt` drift elevated med→high per CodeRabbit review). Grouped into 5 themes below
...
| E3 | ~~**Rate-limit persistence.**~~ Resolved in B4: limits count in Postgres (`action_rate_limit`) on the database clock, shared by every instance. The in-memory bucket is used only in E2E test mode (no database) and as the fallback when the database cannot store a count. | eng | med | 03 OQ3 |
...
| E15 | **Family-tree cycle guard (source bug).** Add a `visited` Set to the `matchIds` ancestor-promotion walk (and harden `buildTree`) to prevent an infinite loop on a cyclic `inviterId` chain. Currently undefended (presumed domain-impossible). **Must fix before shipping — not optional** (CodeRabbit elevated). | eng | high | 13 OQ1 |
