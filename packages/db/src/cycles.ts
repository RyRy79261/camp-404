...
// The year namespace, read side. A "cycle" is one burn year; see
...
/**
 * The cycle to stamp on an activation being opened right now. Read once at Send
...
export async function currentCycleNumber(): Promise<number> {
  ...
  const [row] = await db
    ...
    .from(campSettings)
    .limit(1);
  return currentCycle(resolveCycles(row?.config)).number;
}
