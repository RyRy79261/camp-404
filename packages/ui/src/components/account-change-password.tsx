...
// Change-password form, copied from the AfrikaBurn contributors app's
// @quagga/ui. Backed by Better Auth's `change-password`, which re-authenticates
// with the current password server-side — we never verify a password ourselves.
//
// ONE new-password field (no confirm-twice), a show/hide toggle, paste allowed,
// length-based strength.
//
// THE POLICY IS INJECTED, not imported. The host hands in `assess`, built on
// the same PASSWORD_MIN_LENGTH the auth server enforces. The client check is a
...
/**
 * What a password check returns. `error`
